...
export default function AttendanceManager() {
    ...
    useEffect(() => {
        const fetchAttendance = async () => {
            if (!firestore || !classId || !selectedDate || studentsInClass.length === 0) {
                 setAttendance(new Map(studentsInClass.map(s => [s.id, 'Presente'])));
                return;
            };
    ...
    const handleSaveAttendance = async () => {
        ...
        attendance.forEach((status, studentId) => {
            ...
            if (status === 'Ausente' || status === 'Justificado') {
                const record: AttendanceRecord = { id: attendanceId, studentId, classId, date: formattedDate, status };
        ...
        try {
            await batch.commit();
            toast({ title: "Frequência Salva!", description: `A lista de chamada para ${format(selectedDate, 'dd/MM/yyyy')} foi salva com sucesso.` });
        } catch (error) {
    ...
    const counts = useMemo(() => {
        ...
        attendance.forEach(status => { statusCounts[status]++; });
        return statusCounts;
    }, [attendance]);
    ...
    return (
        <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                        <Select value={filters.ensino} onValueChange={(v) => handleFilterChange('ensino', v)} disabled={isLoadingOptions}>
                            <SelectTrigger><SelectValue placeholder={isLoadingOptions ? "A carregar..." : "Ensino..."} /></SelectTrigger>
                            <SelectContent>{uniqueFilterOptions.ensinos.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                        </Select>
                        <Select value={filters.serie} onValueChange={(v) => handleFilterChange('serie', v)} disabled={isLoadingOptions || !filters.ensino}>
                            <SelectTrigger><SelectValue placeholder={isLoadingOptions ? "A carregar..." : "Série..."} /></SelectTrigger>
                            <SelectContent>{uniqueFilterOptions.series.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                        </Select>
                        <Select value={filters.classe} onValueChange={(v) => handleFilterChange('classe', v)} disabled={isLoadingOptions || !filters.serie}>
                            <SelectTrigger><SelectValue placeholder={isLoadingOptions ? "A carregar..." : "Classe..."} /></SelectTrigger>
                            <SelectContent>{uniqueFilterOptions.classes.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                        </Select>
                        <Select value={filters.turno} onValueChange={(v) => handleFilterChange('turno', v)} disabled={isLoadingOptions || !filters.classe}>
                            <SelectTrigger><SelectValue placeholder={isLoadingOptions ? "A carregar..." : "Turno..."} /></SelectTrigger>
            ...
            {isClassSelected ? (
                        ...
                        <CardHeader>
                            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                <div>
                                    ...
                                    <CardDescription>Turma: {filters.serie} {filters.classe} ({filters.turno})</CardDescription>
                                </div>
                                <div className="flex items-center gap-4 text-sm font-medium">
                                    <span className="flex items-center gap-1 text-green-600"><UserCheck size={16}/> {counts.Presente}</span>
                                    <span className="flex items-center gap-1 text-red-600"><UserX size={16}/> {counts.Ausente}</span>
                                    <span className="flex items-center gap-1 text-yellow-600"><User size={16}/> {counts.Justificado}</span>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent>
                                ...
                                {studentsInClass.map((student) => (
                                    <div key={student.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 border rounded-md hover:bg-muted/50 transition-colors">
                                        <div className="flex items-center gap-2 mb-3 sm:mb-0">
                                            ...
                                            {student.nee && <Badge variant="destructive" className="text-[10px] h-5">AEE</Badge>}
                                        </div>
                                        <RadioGroup value={attendance.get(student.id) || 'Presente'} onValueChange={(value) => handleStatusChange(student.id, value as AttendanceStatus)} className="flex items-center gap-4">
                                            <div className="flex items-center space-x-2"><RadioGroupItem value="Presente" id={`p-${student.id}`} /><Label htmlFor={`p-${student.id}`} className="text-green-600">Presente</Label></div>
                                            <div className="flex items-center space-x-2"><RadioGroupItem value="Ausente" id={`a-${student.id}`} /><Label htmlFor={`a-${student.id}`} className="text-red-600">Ausente</Label></div>
                                            <div className="flex items-center space-x-2"><RadioGroupItem value="Justificado" id={`j-${student.id}`} /><Label htmlFor={`j-${student.id}`} className="text-yellow-600">Justificado</Label></div>
                                        </RadioGroup>
                                    </div>
                                ))}
                            </div>
                            <div className="mt-6 flex justify-end">
                                <Button onClick={handleSaveAttendance} disabled={isSaving}>{isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />} Salvar</Button>
                            </div>
                        </CardContent>
                    </Card>
                ) : (
                     <div className="p-6 text-center h-64 flex flex-col items-center justify-center border-2 border-dashed rounded-lg bg-card/50">
                ...
                <div className="p-6 text-center h-64 flex flex-col items-center justify-center border-2 border-dashed rounded-lg bg-card/50">
                    <Users className="mx-auto h-12 w-12 text-muted-foreground opacity-20" />
                    <h3 className="mt-4 text-lg font-medium">Aguardando Seleção de Turma</h3>
                </div>
            )}
        </div>
    );
