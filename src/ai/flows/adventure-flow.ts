...
// O nome do fluxo precisa ser único
const generateAdventureStepFlow = ai.defineFlow(
  ...
  async (input) => {
    const { genre, character, previousStory, choice } = input;
    ...
    const prompt = `
      ...
      Continue a história com base na escolha do jogador (ou comece a história, se for o primeiro passo).
