...
import { ArrowLeft, BookOpen, Loader2, Dices, Heart, Star, Sword, Wand2, Sparkles, Book, Scroll, Shield, Search, Space, Swords, Info } from 'lucide-react';
...
export default function AventurasFantasticasPage() {
    ...
    const processNode = useCallback((nodeId: string | number) => {
        if (!selectedBook) return;
        const node = selectedBook.nodes[nodeId];
        if (!node) {
        ...
        if (node.staminaChange) {
            ...
            toast({ title: `Energia alterada em ${node.staminaChange}!` });
        }
        if (node.getItems) {
        ...
        if (node.event === 'combat' && node.combat) {
            ...
            processNode(node.autoNavigate.to);
                            ...
                            <CardContent>
                                <Tabs defaultValue="stats" className="w-full">
                                    ...
                                    <TabsContent value="stats" className="mt-4 space-y-4">
                                        <StatItem icon={Sword} label="Habilidade" value={playerStats.skill} maxValue={initialPlayerStats.skill} color="text-red-500" />
                                        <StatItem icon={Heart} label="Energia" value={playerStats.stamina} maxValue={initialPlayerStats.stamina} color="text-green-500" />
                                        <StatItem icon={Star} label="Sorte" value={playerStats.luck} maxValue={initialPlayerStats.luck} color="text-yellow-500" />
                                    </TabsContent>
